...
- `build.gradle`
  ...
  - CI workflows build selected `mcVer` targets directly and parallelize the version matrix outside Gradle.
- `.github/workflows/`
  - CI, preview, and release workflows discover `versionProperties/*.properties` targets and build them as parallel `mc_version` matrix jobs.
  - `reusable-build.yml` builds exactly one selected Minecraft version per invocation.
...
Build only the Minecraft version targets affected by the current change. Do not add or use a Gradle task that sequentially builds every version; CI should parallelize version builds with a workflow matrix instead.
...
- When adding a Minecraft version:
  ...
  6. Only run additional affected version targets when the change touches shared behavior.
